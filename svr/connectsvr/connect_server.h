...
#define RECV_BUF_LEN    (CONNECT_HEAD_LEN + MAX_CS_PKG_BUF_LEN * 2)   // at least 2 package
...
private:
    ...
    bool SocketPoolInit();
    stCliSocket* SocketCreate();
    stCliSocket* SocketGet(uint16 pos) { return &_pool[pos]; }
    ...
    void DealNetwork();
    void AcceptConnect();
    void RecvClientData(stCliSocket& socket);
    void SendClientData();
    void NotifyLogicDiscon(const stCliSocket& socket);
private:
    ...
    char _send_buf[MAX_CONNECT_BUF_LEN];
    uint32 _send_len;
};
